...
# Collections
music_guesser_rooms = None
try:
    if db is not None:
        music_guesser_rooms = db.music_guesser_rooms
        music_guesser_rooms.create_index('code', unique=True)
        music_guesser_rooms.create_index('created_at', expireAfterSeconds=86400)
except Exception as e:
    log.error('Failed to init music_guesser_rooms: %s', e)
    music_guesser_rooms = None

# Helper functions
def _music_guesser_strip_for_player(room, username):
    """Remove sensitive data from room for a specific player"""
    if room.get('state') == 'playing':
        current_index = room.get('data', {}).get('current_index', -1)
        songs = room.get('data', {}).get('songs', [])
        if 0 <= current_index < len(songs):
            room['data']['current_song'] = songs[current_index]
        room['data'].pop('songs', None)
        room['data'].pop('all_songs', None)
    
    if room.get('state') == 'playing':
        submissions = room.get('data', {}).get('song_submissions', {})
        filtered = {}
        for song_id, guesses in submissions.items():
            if username in guesses:
                filtered[song_id] = {username: guesses[username]}
        room['data']['song_submissions'] = filtered
    
    return room

def _music_guesser_shuffle_songs(songs):
    shuffled = songs.copy()
    import random
...
@app.route('/api/music-guesser/room', methods=['POST'])
def music_guesser_create_room():
    username = current_user()
    if not username:
        return jsonify({'error': 'Login required'}), 401
    
    if music_guesser_rooms is None:
        return jsonify({'error': 'Database unavailable'}), 500
    
    import random
    for _ in range(10):
        code = ''.join([str(random.randint(0, 9)) for _ in range(6)])
        if not music_guesser_rooms.find_one({'code': code}):
            break
    else:
        return jsonify({'error': 'Failed to generate unique code'}), 500
    
    data = request.json or {}
    songs_per_player = int(data.get('songs_per_player', 3))
    play_time = int(data.get('play_time', 5))
    
    room = {
        'code': code,
        'host': username,
        'game_type': 'music_guesser',
        'players': [{'username': username, 'ready': False}],
        'state': 'lobby',
        'round': 0,
        'data': {
            'songs_per_player': songs_per_player,
            'play_time': play_time,
            'player_songs': {},
            'songs': [],
            'all_songs': [],
    ...
    music_guesser_rooms.insert_one(room)
    return jsonify({'code': code}), 201

@app.route('/api/music-guesser/room/<code>', methods=['GET'])
def music_guesser_get_room(code):
    username = current_user()
    if not username:
        return jsonify({'error': 'Login required'}), 401
    ...
    room = music_guesser_rooms.find_one({'code': code}, {'_id': 0})
    if not room:
        return jsonify({'error': 'Room not found'}), 404
    
    if room['state'] == 'playing':
        room = _music_guesser_strip_for_player(room, username)
    
    return jsonify(room)

@app.route('/api/music-guesser/room/<code>/join', methods=['POST'])
def music_guesser_join_room(code):
    username = current_user()
    if not username:
        return jsonify({'error': 'Login required'}), 401
    ...
    room = music_guesser_rooms.find_one({'code': code})
    if not room:
        return jsonify({'error': 'Room not found'}), 404
    ...
    if len(room['players']) >= 8:
        return jsonify({'error': 'Room full (max 8 players)'}), 400
    
    music_guesser_rooms.update_one(
        {'code': code},
        {'$push': {'players': {'username': username, 'ready': False}}}
    )
    
    socketio.emit('music_guesser_player_joined', {'username': username}, to=code)
    
    return jsonify({'code': code})

@app.route('/api/music-guesser/room/<code>/action', methods=['POST'])
def music_guesser_room_action(code):
    username = current_user()
    if not username:
        return jsonify({'error': 'Login required'}), 401
    ...
    if action == 'set_song_count':
        if room['host'] != username:
            return jsonify({'error': 'Only host can change settings'}), 403
        count = int(data.get('count', 3))
        if count < 1 or count > 6:
            return jsonify({'error': 'Songs per player must be 1-6'}), 400
        update['data.songs_per_player'] = count
        music_guesser_rooms.update_one({'code': code}, {'$set': update})
        return jsonify({'success': True})
    
    if action == 'set_play_time':
        if room['host'] != username:
            return jsonify({'error': 'Only host can change settings'}), 403
        play_time = int(data.get('time', 5))
        if play_time < 3 or play_time > 15:
            return jsonify({'error': 'Play time must be 3-15 seconds'}), 400
        update['data.play_time'] = play_time
        music_guesser_rooms.update_one({'code': code}, {'$set': update})
        return jsonify({'success': True})
    ...
    if action == 'submit_song':
        ...
        songs_per_player = room.get('data', {}).get('songs_per_player', 3)
        if len(player_songs[username]) >= songs_per_player:
            return jsonify({'error': f'You can only submit {songs_per_player} songs'}), 400
        ...
        all_players = [p['username'] for p in room['players']]
        songs_per = room.get('data', {}).get('songs_per_player', 3)
        all_submitted = all(
            len(player_songs.get(p, [])) >= songs_per
            for p in all_players
        )
        ...
        if all_submitted and len(all_players) >= 2:
            ...
            for p, songs in player_songs.items():
                all_songs.extend(songs)
            shuffled = _music_guesser_shuffle_songs(all_songs)
            
            scores = {p: 0 for p in all_players}
            
            update['data.songs'] = shuffled
            update['data.all_songs'] = shuffled
            update['data.scores'] = scores
            update['data.current_index'] = 0
            update['data.state'] = 'playing'
            update['state'] = 'playing'
            update['round'] = 1
            update['data.guess_history'] = []
            update['data.song_submissions'] = {}
            update['data.has_buzzed'] = {}
            update['data.show_reveal'] = False
            update['data.position'] = 0
            update['data.played_at'] = datetime.now(timezone.utc).isoformat()
            update['data.buzz_info'] = None
            
            socketio.emit('music_guesser_game_started', {'songs_count': len(shuffled)}, to=code)
    ...
    if action in ('play', 'pause', 'seek'):
        ...
        if action == 'play':
            ...
            update['data.position'] = data.get('position', 0)
            update['data.played_at'] = datetime.now(timezone.utc).isoformat()
            socketio.emit('music_guesser_play', {'position': data.get('position', 0)}, to=code)
        
        elif action == 'pause':
            update['data.state'] = 'paused'
            update['data.position'] = data.get('position', 0)
            socketio.emit('music_guesser_pause', {'position': data.get('position', 0)}, to=code)
        
        elif action == 'seek':
            update['data.position'] = data.get('position', 0)
            update['data.played_at'] = datetime.now(timezone.utc).isoformat()
            socketio.emit('music_guesser_seek', {'position': data.get('position', 0)}, to=code)
    ...
    if action == 'buzz':
        ...
        has_buzzed = room.get('data', {}).get('has_buzzed', {})
        if song_id in has_buzzed and username in has_buzzed[song_id]:
            return jsonify({'error': 'You already buzzed for this song'}), 400
        
        if current_song.get('addedBy') == username:
            return jsonify({'error': 'You can\'t guess your own song!'}), 400
        
        if song_id not in has_buzzed:
            has_buzzed[song_id] = []
        has_buzzed[song_id].append(username)
        update['data.has_buzzed'] = has_buzzed
        
        update['data.state'] = 'paused'
        update['data.position'] = data.get('position', 0)
        update['data.buzz_info'] = {
            'username': username,
            'song_id': song_id,
        ...
        music_guesser_rooms.update_one({'code': code}, {'$set': update})
        socketio.emit('music_guesser_buzz', {'username': username}, to=code)
    ...
    if action == 'submit_guess':
        ...
        song_submissions = room.get('data', {}).get('song_submissions', {})
        if song_id in song_submissions and username in song_submissions[song_id]:
            return jsonify({'error': 'You already submitted a guess for this song'}), 400
        
        if song_id not in song_submissions:
            song_submissions[song_id] = {}
        song_submissions[song_id][username] = {
            'guess': guess,
            'verified': False,
            'correct': None,
            'timestamp': datetime.now(timezone.utc).isoformat()
        }
        update['data.song_submissions'] = song_submissions
        
        songs = room.get('data', {}).get('songs', [])
        current_song = next((s for s in songs if s.get('id') == song_id), None)
        
        if current_song:
            update['data.guess_submitted'] = {
                'guesser': username,
                'guess': guess,
                'song_id': song_id,
                'song_owner': current_song.get('addedBy')
            }
            update['data.verify_guess'] = {
                'guesser': username,
                'guess': guess,
                'song_id': song_id
            }
            
            socketio.emit('music_guesser_guess_submitted', {
                'guesser': username,
                'guess': guess,
                'song_id': song_id
            }, to=current_song.get('addedBy'))
        
        music_guesser_rooms.update_one({'code': code}, {'$set': update})
        return jsonify({'success': True})
    
    # ===== VERIFY GUESS =====
    if action == 'verify_guess':
        song_id = data.get('song_id')
        guess_text = data.get('guess', '').strip()
        is_correct = data.get('correct', False)
        
        if not song_id or not guess_text:
            return jsonify({'error': 'Song ID and guess required'}), 400
        
        songs = room.get('data', {}).get('songs', [])
        current_song = next((s for s in songs if s.get('id') == song_id), None)
        if not current_song or current_song.get('addedBy') != username:
            return jsonify({'error': 'Only the song owner can verify'}), 403
        
        song_submissions = room.get('data', {}).get('song_submissions', {})
        verified = False
        guesser = None
        
        if song_id in song_submissions:
            for g_user, g_data in song_submissions[song_id].items():
                if g_data.get('guess') == guess_text and not g_data.get('verified'):
                    g_data['verified'] = True
                    g_data['correct'] = is_correct
                    verified = True
                    guesser = g_user
                    break
        
        if not verified:
            return jsonify({'error': 'Guess not found or already verified'}), 400
        ...
        guess_history = room.get('data', {}).get('guess_history', [])
        ...
        update['data.guess_history'] = guess_history
        update['data.song_submissions'] = song_submissions
        update['data.guess_submitted'] = None
        update['data.verify_guess'] = None
        update['data.show_reveal'] = True
        
        music_guesser_rooms.update_one({'code': code}, {'$set': update})
        
        socketio.emit('music_guesser_guess_verified', {
            'guesser': guesser,
            'guess': guess_text,
            'correct': is_correct,
            'song_id': song_id
        ...
        return jsonify({'success': True, 'correct': is_correct, 'guesser': guesser})
    ...
    if action == 'next_song':
        ...
        if next_index >= len(songs):
            ...
            winner = max(scores, key=scores.get) if scores else None
            update['data.winner'] = winner
            update['data.final_scores'] = scores
            
            socketio.emit('music_guesser_game_finished', {
                'scores': scores, 
                'winner': winner
            }, to=code)
        else:
            ...
            socketio.emit('music_guesser_next_song', {'index': next_index}, to=code)
    ...
    if action == 'skip_song':
        if room['host'] != username:
            return jsonify({'error': 'Only host can skip songs'}), 403
        ...
        socketio.emit('music_guesser_song_skipped', {
            'index': current_index,
            'song_id': songs[current_index].get('id')
        }, to=code)
    ...
    if action == 'leave':
        if room['host'] == username:
            remaining = [p for p in room['players'] if p['username'] != username]
            if remaining:
                music_guesser_rooms.update_one(
                    {'code': code},
                    {'$set': {'host': remaining[0]['username']}, '$pull': {'players': {'username': username}}}
                )
            else:
                music_guesser_rooms.delete_one({'code': code})
                return jsonify({'deleted': True})
        else:
            music_guesser_rooms.update_one(
                {'code': code},
        ...
        socketio.emit('music_guesser_player_left', {'username': username}, to=code)
        return jsonify({'success': True})
    
    return jsonify({'error': 'Unknown action'}), 400

@app.route('/api/music-guesser/room/<code>/state', methods=['GET'])
def music_guesser_room_state(code):
    username = current_user()
    if not username:
        return jsonify({'error': 'Login required'}), 401
    ...
    state = {
        ...
        'players': [p['username'] for p in room.get('players', [])],
        'host': room.get('host'),
        'data': {
            'current_index': room.get('data', {}).get('current_index', -1),
            'state': room.get('data', {}).get('state', 'stopped'),
            'position': room.get('data', {}).get('position', 0),
            'scores': room.get('data', {}).get('scores', {}),
            'show_reveal': room.get('data', {}).get('show_reveal', False),
            'guess_submitted': room.get('data', {}).get('guess_submitted'),
            'verify_guess': room.get('data', {}).get('verify_guess'),
            'song_submissions': room.get('data', {}).get('song_submissions', {}),
            'guess_history': room.get('data', {}).get('guess_history', []),
            'winner': room.get('data', {}).get('winner'),
            'final_scores': room.get('data', {}).get('final_scores'),
            'play_time': room.get('data', {}).get('play_time', 5),
            'songs_per_player': room.get('data', {}).get('songs_per_player', 3)
        }
    }
    ...
    if 0 <= current_index < len(songs):
        current_song = songs[current_index]
        state['data']['current_song'] = {
            'id': current_song.get('id'),
            'title': current_song.get('title'),
            'artist': current_song.get('artist'),
            'thumbnail': current_song.get('thumbnail'),
            'youtube_query': current_song.get('youtube_query'),
            'addedBy': current_song.get('addedBy')
        }
    
    return jsonify(state)

@app.route('/api/music-guesser/stats/<target_username>', methods=['GET'])
def music_guesser_stats(target_username):
    if music_guesser_rooms is None:
        return jsonify({'error': 'Database unavailable'}), 500
    
    played = music_guesser_rooms.count_documents({
        'players.username': target_username,
        'state': 'finished'
    })
    
    won = music_guesser_rooms.count_documents({
        'data.winner': target_username,
        'state': 'finished'
    })
...
# ===== MUSIC GUESSER SOCKET EVENTS =====

@socketio.on('music_guesser_join')
def on_music_guesser_join(data):
    code = data.get('code')
    username = session.get('username')
    if not code or not username:
        return
    join_room(code)
    emit('music_guesser_joined', {'username': username}, to=code)

@socketio.on('music_guesser_leave')
def on_music_guesser_leave(data):
    code = data.get('code')
    username = session.get('username')
    if not code or not username:
        return
    leave_room(code)
    emit('music_guesser_left', {'username': username}, to=code)

@socketio.on('music_guesser_buzz')
def on_music_guesser_buzz(data):
    code = data.get('code')
    username = session.get('username')
    if not code or not username:
        return
    emit('music_guesser_buzz', {'username': username}, to=code, include_self=False)

@socketio.on('music_guesser_guess')
def on_music_guesser_guess(data):
    code = data.get('code')
    username = session.get('username')
    guess = data.get('guess', '')
    if not code or not username:
        return
    emit('music_guesser_guess', {'username': username, 'guess': guess}, to=code, include_self=False)

@socketio.on('music_guesser_verify')
def on_music_guesser_verify(data):
    code = data.get('code')
    username = session.get('username')
    correct = data.get('correct', False)
    if not code or not username:
        return
    emit('music_guesser_verify', {'username': username, 'correct': correct}, to=code, include_self=False)
